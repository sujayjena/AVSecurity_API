...
namespace AVSecurity.Persistence.Repositories
{
    public class AdminMasterRepository : GenericRepository, IAdminMasterRepository
    {
        ...
        public async Task<int> SaveMaterialStatus(MaterialStatus_Request parameters)
        {
            DynamicParameters queryParameters = new DynamicParameters();

            queryParameters.Add("@Id", parameters.Id);
            queryParameters.Add("@MaterialStatusName", parameters.MaterialStatusName);
            queryParameters.Add("@IsActive", parameters.IsActive);
            queryParameters.Add("@UserId", SessionManager.LoggedInUserId);

            return await SaveByStoredProcedure<int>("SaveMaterialStatus", queryParameters);
        }

        public async Task<IEnumerable<MaterialStatus_Response>> GetMaterialStatusList(MaterialStatus_Search parameters)
        {
            DynamicParameters queryParameters = new DynamicParameters();
            ...
            var result = await ListByStoredProcedure<MaterialStatus_Response>("GetMaterialStatusList", queryParameters);
            parameters.Total = queryParameters.Get<int>("Total");

            return result;
        }

        public async Task<MaterialStatus_Response?> GetMaterialStatusById(int Id)
        {
            DynamicParameters queryParameters = new DynamicParameters();

            queryParameters.Add("@Id", Id);

            return (await ListByStoredProcedure<MaterialStatus_Response>("GetMaterialStatusById", queryParameters)).FirstOrDefault();
        }
